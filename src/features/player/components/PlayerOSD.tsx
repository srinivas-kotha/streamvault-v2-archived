...
export function PlayerOSD({ action }: { action: OSDAction | null }) {
  ...
  useEffect(() => {
    ...
    const hideDelay =
      action.type === "fast-forward" || action.type === "fast-rewind"
        ? 800
        : 1200;
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-40">
      <div
        className={`rounded-2xl px-8 py-6 text-white text-center animate-in fade-in duration-150 ${isTVMode ? "bg-black/85" : "bg-black/70 backdrop-blur-sm"}`}
      >
      ...
      return (
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="w-32 h-1.5 bg-white/20 rounded-full overflow-hidden">
            <div
              className="h-full bg-teal rounded-full transition-[width]"
              style={{ width: `${Math.round((action.value ?? 0) * 100)}%` }}
            />
          </div>
          <span className="text-sm font-medium">
            {Math.round((action.value ?? 0) * 100)}%
          </span>
        </div>
      );
    case "play":
