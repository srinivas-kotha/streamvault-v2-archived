...
export function PlayerControls({
  ...
  return (
    <div className="absolute inset-0 flex flex-col justify-end opacity-0 hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t from-obsidian/80 via-transparent to-obsidian/30">
      {/* Progress bar */}
      {!isLive && duration > 0 && (
      ...
      <div className="flex items-center gap-2 px-4 pb-4 pt-2">
        ...
        {/* Prev/Next */}
        {hasPrev && (
          <button onClick={onPrev} className="p-1.5 text-white/70 hover:text-white transition-colors" title="Previous">
        ...
        {hasNext && (
          <button onClick={onNext} className="p-1.5 text-white/70 hover:text-white transition-colors" title="Next">
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="relative flex items-center" onMouseEnter={() => setShowVolume(true)} onMouseLeave={() => setShowVolume(false)}>
          <button onClick={onMuteToggle} className="p-1.5 text-white/70 hover:text-white transition-colors">
            {isMuted || volume === 0 ? (
        ...
        {qualityLevels.length > 1 && (
          <div className="relative">
            <button onClick={() => setShowQuality(!showQuality)} className="p-1.5 text-white/70 hover:text-white transition-colors">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {/* Mini-player */}
        <button onClick={toggleMiniPlayer} className="p-1.5 text-white/70 hover:text-white transition-colors" title="Mini player">
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {/* PiP */}
        <button onClick={() => playerRef.current?.togglePiP()} className="p-1.5 text-white/70 hover:text-white transition-colors" title="Picture-in-Picture">
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <rect x="2" y="3" width="20" height="14" rx="2" />
        ...
        {/* Fullscreen */}
        <button onClick={() => playerRef.current?.toggleFullscreen()} className="p-1.5 text-white/70 hover:text-white transition-colors" title="Fullscreen">
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
